...
Bioconda is a package repository specifically for bioinformatics and genomics.   Since 2020 there has been significant growth in multi-core Arm-based laptops and desktops, for example the recent introduction of Windows on Arm. Moreover in the coming years, Arm expect many more OEM (original equipment manufacturer) to be available. These devices enable computationally expensive bioinformatics and statistics to be run on local machines. The downstream applications could be diagnosis that is quicker, cheaper and closer to a hospital patient, such as through the pilot [ROBIN software](https://www.nottingham.ac.uk/news/genetic-brain-tumour-diagnosis). Although many top Bioconda packages now support Linux/Arm, there's still a gap for native macOS / Windows on Arm, where many packages default to emulated x86 environments. Similarly, for the R community, Windows-on-Arm support for community created packages is lacking with many packages unable to build due to issues such as x86-specific code.
